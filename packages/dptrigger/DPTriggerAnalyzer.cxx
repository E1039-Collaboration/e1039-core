...
int DPTriggerAnalyzer::InitRun(PHCompositeNode* topNode) 
{
  ...
  string line="";
  std::ifstream fin ("trigger_57.txt", std::ifstream::in);
  if(fin.is_open()) 
    {
...
int DPTriggerAnalyzer::process_event(PHCompositeNode* topNode) {
  ...
  if (!C1X_hits)
    {
      std::cout << Name() << " Could not locate g4 hit node " << "G4HIT_C1X" << endl;
      return Fun4AllReturnCodes::ABORTRUN;
    }
  ...
  if(_hit_vector) {
	...
	if(p_digitizer->getTriggerLv(hit->get_detector_id())<0) continue;
  ...
  if(NIMONLY) return Fun4AllReturnCodes::ABORTRUN;
  ...
  if(_hit_vector) {
    ...
    if(!buildHitPattern(n_FPGA_hits, uniqueIDs)) return Fun4AllReturnCodes::ABORTEVENT;
