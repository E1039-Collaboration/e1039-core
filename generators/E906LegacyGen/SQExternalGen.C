...
SQExternalGen::SQExternalGen(const std::string& name):
...
SQExternalGen::~SQExternalGen()
{
  if(_vertexGen != nullptr) delete _vertexGen;
}

int SQExternalGen::Init(PHCompositeNode* topNode)
{  
  return Fun4AllReturnCodes::EVENT_OK;
}

int SQExternalGen::InitRun(PHCompositeNode* topNode)
{
  ...
  if(!_extFile)
  {
    std::cout << "SQExternalGen: cannot find the input ROOT file, abort " << _extFileName << std::endl;
    exit(EXIT_FAILURE);
  }
...
int SQExternalGen::process_event(PHCompositeNode* topNode)
{

  setBucketSize((int)_beam_intensity_profile->GetRandom()*30);
  ...
  for(auto par = _extParticles.begin(); par != _extParticles.end(); ++par)
  {
    if(par->_evtID != eventID_curr)
    {
     ...
     // priVtx = _vertexGen->generateVertex();
    }

    //int vtxidx = _ineve->AddVtx(priVtx.X()+par->_pos.X(), priVtx.Y()+par->_pos.Y(), priVtx.Z()+par->_pos.Z(), 0.);
  ...
  _mcevt->set_cross_section(_bucketSize);
   ...
   std::cout<<"bucket size in external generator: "<<_bucketSize<<std::endl;
   _n_proc_evt++;
  _evt->set_run_id(0);
  _evt->set_spill_id(0);
...
bool SQExternalGen::readExtTree(int nEvents)
{
  _extParticles.clear();
