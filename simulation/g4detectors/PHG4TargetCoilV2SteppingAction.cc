...
PHG4TargetCoilV2SteppingAction::PHG4TargetCoilV2SteppingAction(PHG4TargetCoilV2Detector* detector, const PHParameters* parameters)
  : detector_(detector)
  , params(parameters)
  , hits_(nullptr)
  , hit(nullptr)
...
PHG4TargetCoilV2SteppingAction::~PHG4TargetCoilV2SteppingAction()
{
...
//____________________________________________________________________________..
bool PHG4TargetCoilV2SteppingAction::UserSteppingAction(const G4Step* aStep, bool)
{
  ...
  if (active)
  {
	...
	cout << " previous phys pre vol: " << savevolpre->GetName()
      ...
      if (hit->get_z(0) * cm > zmax || hit->get_z(0) * cm < zmin)
      {
        cout << detector_->SuperDetector() << std::setprecision(9)
             << "PHG4TargetCoilV2SteppingAction: Entry hit z " << hit->get_z(0) * cm
    ...
    {
      cout << detector_->SuperDetector() << std::setprecision(9)
           << " PHG4TargetCoilV2SteppingAction: Exit hit z " << hit->get_z(1) * cm
           << " outside acceptance zmin " << zmin
           << ", zmax " << zmax << ", layer: " << layer_id << endl;
    }
    if (geantino)
...
//____________________________________________________________________________..
void PHG4TargetCoilV2SteppingAction::SetInterfacePointers(PHCompositeNode* topNode)
{
  string hitnodename;
  ...
  {
    std::cout << "PHG4TargetCoilV2SteppingAction::SetTopNode - unable to find " << hitnodename << std::endl;
  }
}
