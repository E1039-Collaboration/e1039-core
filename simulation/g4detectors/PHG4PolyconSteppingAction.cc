...
PHG4PolyconSteppingAction::PHG4PolyconSteppingAction(PHG4PolyconDetector* detector, const PHParameters* parameters)
  : detector_(detector)
  , params(parameters)
  , hits_(nullptr)
  , hit(nullptr)
...
PHG4PolyconSteppingAction::~PHG4PolyconSteppingAction()
{
...
//____________________________________________________________________________..
bool PHG4PolyconSteppingAction::UserSteppingAction(const G4Step* aStep, bool)
{
  ...
  if (active)
  {
	...
	cout << " previous phys pre vol: " << savevolpre->GetName()
      ...
      if (hit->get_z(0) * cm > zmax || hit->get_z(0) * cm < zmin)
      {
        cout << detector_->SuperDetector() << std::setprecision(9)
             << "PHG4PolyconSteppingAction: Entry hit z " << hit->get_z(0) * cm
    ...
    {
      cout << detector_->SuperDetector() << std::setprecision(9)
           << " PHG4PolyconSteppingAction: Exit hit z " << hit->get_z(1) * cm
           << " outside acceptance zmin " << zmin
           << ", zmax " << zmax << ", layer: " << layer_id << endl;
    }
    if (geantino)
...
//____________________________________________________________________________..
void PHG4PolyconSteppingAction::SetInterfacePointers(PHCompositeNode* topNode)
{
  string hitnodename;
  ...
  {
    std::cout << "PHG4PolyconSteppingAction::SetTopNode - unable to find " << hitnodename << std::endl;
  }
}
